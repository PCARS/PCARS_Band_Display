...
/****************************** Determine if band conflict exists  ****************************************/ 
  ...
  if ( band_1 == band_2 )  // For alarm to trigger both radios must be tuned to the same band.
  {
    ...
    radio1_active = query_Radio( Serial1 );  // Check if radio # 1 is still active

    radio2_active = query_Radio( Serial2 );  // Check if radio # 2 is still active
    ...
    conflict_confirmed = false;  // Reset band conflict flag if operating bands are no longer the same
...
bool query_Radio( HardwareSerial &radio )
{
